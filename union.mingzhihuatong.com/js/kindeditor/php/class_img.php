...
//$img1 = '1.jpg';
// 水印后的图片
//$img2 = './images/test_new.jpg';
// 水印
//$water = 'water.png';
//$img = new image();
// 等比缩放
//$img->param($img1)->thumb('./images/test_0.jpg', 200,200,0);
// center center 裁剪
//$img->param($img1)->thumb('./images/test_1.jpg', 200,200,1);
// top left 裁剪
//$img->param($img1)->thumb('./images/test_2.jpg', 200,200,2);
// 右下角添加水印
    ...
    // water(保存地址,水印图片,水印位置,透明度)
        public function water($filename,$water,$pos=0,$pct=80){
            ...
            // 设定图像的混色模式
                ...
                imagecopy($oldimg, $waterimg, $posX, $posY, 0, 0, $water_w,$water_h);
                $fun = 'image'.$type;
                switch($type) {
                case 'jpeg':
                    !imagejpeg($oldimg, $filename, 95) && die('保存失败!检查目录是否存在并且可写?');
                    break;
                case 'png':
                    !imagepng($oldimg, $filename, 0) && die('保存失败!检查目录是否存在并且可写?');
                    break;
                case 'gif':
                    !imagegif($oldimg, $filename) && die('保存失败!检查目录是否存在并且可写?');
                    break;
                }
                imagedestroy($oldimg);
